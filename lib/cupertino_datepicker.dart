...
class CupertinoDatePicker extends StatefulWidget {
  ...
  static double _getColumnWidth(
    ...
    return painter.maxIntrinsicWidth*1.2;
  }
}
...
class _CupertinoDatePickerDateState extends State<CupertinoDatePicker> {
  ...
  FixedExtentScrollController dayController,monthController,yearController;
    ...
    return CupertinoPicker(
      ...
      children: List<Widget>.generate(31, (int index) {
        ...
        if(widget.minimumDate!=null) {
          int minYear = widget.minimumDate.year;
          int minMonth = widget.minimumDate.month;
          int minDay = widget.minimumDate.day;

          // Null if not out of range.
          if (minYear== selectedYear&& minMonth == selectedMonth && index < minDay -1) {
            disableTextStyle =
  ...
  Widget _buildMonthPicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    ...
    return CupertinoPicker(
      scrollController: monthController,//FixedExtentScrollController(initialItem: selectedMonth - 1),
      offAxisFraction: offAxisFraction,
      itemExtent: _kItemExtent,
      useMagnifier: _kUseMagnifier,
      magnification: _kMagnification,
      ...
      children: List<Widget>.generate(12, (int index) {
        ...
        if(widget.minimumDate!=null) {
          ...
          int minMonth = widget.minimumDate.month;
          // Null if not out of range.
          if (minYear== selectedYear&& index < minMonth-1) {
            disableTextStyle =
            const TextStyle(color: CupertinoColors.inactiveGray);
          }
        }
        return itemPositioningBuilder(
          context,
          Text(localizations.datePickerMonth(index + 1),style: disableTextStyle,),
        );
      }),
      looping: true,
    );
  ...
  Widget _buildYearPicker(double offAxisFraction, TransitionBuilder itemPositioningBuilder) {
    return CupertinoPicker.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return itemPositioningBuilder(
          context,
          Text(localizations.datePickerYear(index),style: disableTextStyle,),
        );
      },
    );
  }
